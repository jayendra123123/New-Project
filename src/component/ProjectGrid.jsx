...
const ProjectGrid = () => {
  return (
    <section id="work" className="w-full bg-white dark:bg-[#221f10] py-12 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8 border-t-4 border-dark-text dark:border-white/10">
      <div className="max-w-[1200px] mx-auto flex flex-col gap-8 sm:gap-10 md:gap-12">
        <div className="flex flex-col gap-4 sm:gap-6 border-b-2 border-dashed border-dark-text/20 dark:border-white/20 pb-4 sm:pb-6">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-dark-text dark:text-white tracking-tight">My work</h2>
          </div>
          <a 
            className="flex items-center gap-2 text-sm sm:text-base md:text-lg font-bold text-dark-text dark:text-white hover:text-primary transition-colors group w-fit" 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {projects.map((project, idx) => (
            <div 
              key={idx}
              className={`group relative flex flex-col bg-white dark:bg-bg-card-dark p-3 sm:p-4 pb-6 rounded-lg sm:rounded-xl border-2 border-dark-text dark:border-white/20 shadow-comic hover:shadow-comic-hover hover:-translate-y-2 hover:rotate-0 transition-all duration-300 transform ${project.rotation}`}
            >
              <div 
                className="aspect-[4/3] w-full rounded-lg bg-cover bg-center border-2 border-dark-text mb-3 sm:mb-4 overflow-hidden relative"
              ...
              <div className="px-1 sm:px-2 flex flex-col flex-grow">
                ...
                <h3 className="text-lg sm:text-xl md:text-2xl font-black leading-tight mb-1 sm:mb-2 group-hover:text-primary transition-colors dark:text-white">{project.title}</h3>
                <p className="text-xs sm:text-sm font-medium text-dark-text/70 dark:text-white/70 line-clamp-2 mb-4">{project.description}</p>
                ...
                <div className="flex gap-2 sm:gap-3 pt-4 mt-auto">
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="flex-1 flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-4 py-2 bg-white dark:bg-black border-2 border-dark-text dark:border-white/20 rounded-lg shadow-comic-sm hover:shadow-comic hover:-translate-y-1 transition-all text-xs sm:text-sm font-bold text-dark-text dark:text-white"
                    >
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="flex-1 flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-4 py-2 bg-primary border-2 border-dark-text rounded-lg shadow-comic-sm hover:shadow-comic hover:-translate-y-1 transition-all text-xs sm:text-sm font-bold text-dark-text"
                    >
